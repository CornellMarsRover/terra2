...
id=16 auger) per ``src/astrotech_rover/scripts/auger_keys_test_harness.py``. moteus
...
The real-hardware path lives in :mod:`astrotech_rover.drivers.auger_real`
and is the default. This mock is opt-in via ``URC_AUGER_MOCK=1`` in
:mod:`astrotech_rover.astrotech_node`, for GUI dev without an fdcanusb /
rover hardware.

TODO: the real moteus mode/fault values are
