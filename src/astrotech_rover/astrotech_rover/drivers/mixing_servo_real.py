...
Default mixing-servo driver in :mod:`astrotech_rover.astrotech_node`.
Drop-in counterpart of `MockMixingServoDriver` (same `__init__(node, cfg)`
signature, same advertised topics/services); set `URC_MIXING_SERVO_MOCK=1`
to swap in the simulation driver for hardware-free dev.

Code path
---------
Mirrors `src/astrotech_rover/scripts/mixing_servo_min.py` byte-for-byte:
...
with EBUSY. On a single fdcanusb, mock one of them
(`URC_AUGER_MOCK=1` or `URC_MIXING_SERVO_MOCK=1`) so only one driver holds
the bus, until a unified driver-node lands. The user's plan is to physically separate the two
...
# Vendored CMR_CANFD library lives in this package at
# astrotech_rover/third_party/astrotech_canfd/ (NOT pip-installed).
# Resolving the path needs `realpath` on __file__ because
# `colcon build --symlink-install` makes the installed driver a symlink to
# the source -- realpath gives us the actual on-disk source path, from
# which we walk up to the package root (src/astrotech_rover/).
#
# This file: src/astrotech_rover/astrotech_rover/drivers/mixing_servo_real.py
#   dirname               -> .../astrotech_rover/astrotech_rover/drivers
#   + "../.."             -> .../src/astrotech_rover   (package root)
#
...
_VENDOR_DIR = os.path.join(
    _PKG_ROOT, "third_party", "astrotech_canfd",
...
class RealMixingServoDriver:
    ...
    async def _async_main(self) -> None:
        ...
        try:
            ...
            # Same six commands as src/astrotech_rover/scripts/mixing_servo_min.py.
            for cmd in (
                "can off",
