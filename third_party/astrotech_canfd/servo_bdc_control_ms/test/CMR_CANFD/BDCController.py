...
class BDCController:
    ...
    # Duration sent in seconds in general (duration_units = 1)
    # Duration sent in ms for alternate forward and reverse (duration_units = 0)
    ...
    async def move_motor_forward_ms(self, Duration: int):
        frame = self._make_control_frame(Msg_Type=1, Duration_Units=0, Duration= Duration, Clear_Faults=0, Query_data=1, Mode_Select=2)
        await self.can.write_frame(std_id=self.can_id, data_hex=frame.hex(), flags="FB")

    async def move_motor_reverse_ms(self, Duration: int):
        frame = self._make_control_frame(Msg_Type=3, Duration_Units=0, Duration= Duration, Clear_Faults=0, Query_data=1, Mode_Select=3)
        await self.can.write_frame(std_id=self.can_id, data_hex=frame.hex(), flags="FB")
