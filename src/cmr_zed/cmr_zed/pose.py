...
class ZedPose(Node):
    def __init__(self):
        ...
        self.get_logger().info('ZedPose node has been started.')
    ...
    def publish_pose(self):
        ...
        if self.zed.get_position(self.current_pose, sl.REFERENCE_FRAME.WORLD) == sl.POSITIONAL_TRACKING_STATE.OK:
            ...
            # Translations
            pose_msg.twist.linear.x = translation[0] - self.current_pos[0]
            pose_msg.twist.linear.y = translation[1] - self.current_pos[1]
            pose_msg.twist.linear.z = translation[2] - self.current_pos[2]
            self.current_pos = (translation[0], translation[1], translation[2])
            self.get_logger().info(f"xy: {self.current_pos}")
            # Rotations
