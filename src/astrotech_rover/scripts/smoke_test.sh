#!/usr/bin/env bash
# Smoke test for the Astrotech rover node (mock mode — no hardware).
#
# Builds cmr_msgs + astrotech_rover, launches it with all features in MOCK
# mode (URC_*_MOCK=1), verifies every expected topic appears, checks two
# rates, and calls one service. Mock mode is required here: the default is
# real drivers, which idle (publish nothing) when no hardware is attached.
#
...
#
#     bash src/astrotech_rover/scripts/smoke_test.sh
#
# Each check is numbered and prefixed with its step from the Phase 2a brief.

set -euo pipefail

# This script lives at src/astrotech_rover/scripts/; repo root is three up.
...
log "colcon build --packages-select cmr_msgs astrotech_rover"
colcon build \
...
# --- Step 2: launch the rover node (all features mocked) in the background ---
LOG_DIR="$(mktemp -d)"
LAUNCH_LOG="${LOG_DIR}/mock_launch.log"
log "launching astrotech.launch.py with all features in MOCK mode (logs in ${LAUNCH_LOG})"
URC_AUGER_MOCK=1 URC_MIXING_SERVO_MOCK=1 URC_RAMAN_MOCK=1 URC_ENV_MOCK=1 \
    ros2 launch astrotech_rover astrotech.launch.py >"${LAUNCH_LOG}" 2>&1 &
MOCK_PID=$!
cleanup() {
