...
namespace detail
{
  ...
  // Specialization for goal response error value
  template <>
  struct ActionDefaultReturnConstruct<rclcpp_action::GoalResponse> {
    inline static constexpr auto val = rclcpp_action::GoalResponse::REJECT;
  };

  // Specialization for cancel response error value
  template <>
  struct ActionDefaultReturnConstruct<rclcpp_action::CancelResponse> {
    inline static constexpr auto val = rclcpp_action::CancelResponse::REJECT;
  };
}  // namespace detail
...
public:
  ...
  template <typename T>
  using make_return_t = detail::ActionDefaultReturnConstruct<T>;
  ...
  const typename rclcpp_action::Server<ActionT>::AcceptedCallback& accepted_callback)
{
  ...
  return std::make_unique<LifecycleActionServer<ActionT>>(
    node, [&node]() { node.error_transition(); }, config,
    goal_callback, cancel_callback, accepted_callback);
}
