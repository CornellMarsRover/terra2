...
class FaultHandler : public rclcpp::Node
{
  ...
  private:
    ...
    // process
    // TODO(fad35): not sure that this mutex is needed, because we are using a
    // single-threaded executor and so the timer and service callbacks might just be
    // executed in series. but maybe it's worth leaving here to be agnostic to the
    // kind of executor we're dealing with?
    ...
    void initialize()
    {
        ...
        auto timer_callback = [this]() {
            ...
            for (auto it = m_nodes_to_restart.cbegin();
                 it != m_nodes_to_restart.cend();) {
                auto node_name = it->first;
                auto delay = it->second;
                if (delay > 0) {
                    delay--;
                    m_nodes_to_restart[node_name] = delay;
                    it++;
                } else {
        ...
        m_timer = this->create_wall_timer(1s, timer_callback);
