...
class TypingMission(Node):
    ...
    def __init__(self):
        ...
        # OpenCV 4.7 introduced DetectorParameters(); older installations use
        # DetectorParameters_create().  Support both rover/laptop versions.
        if hasattr(cv2.aruco, "DetectorParameters"):
            self.detector_params = cv2.aruco.DetectorParameters()
        else:
            self.detector_params = cv2.aruco.DetectorParameters_create()

        # IDs printed on the four physical keyboard-corner markers.
        self.corner_tag_ids = {1, 2, 3, 4}
