...
#include "cmr_nav/ar_tag_navigate.hpp"

namespace nav2_bt_navigator
{

bool ARTagNavigator::configure(
    ...
    std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
    ...
    goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
        ...
        std::bind(&NavigateToPoseNavigator::onGoalPoseReceived, this,
                  std::placeholders::_1));
    return true;
}
    ...
    rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
    ...
    if (!node->has_parameter("default_ar_tag_bt_xml")) {
        std::string pkg_share_dir =
            ament_index_cpp::get_package_share_directory("ar_tag_navigator");
        node->declare_parameter<std::string>("default_ar_tag_bt_xml",
                                             pkg_share_dir + "/bt/ar_tag_bt.xml");
    }

    node->get_parameter("default_ar_tag_bt_xml", default_bt_xml_filename);
...
void ARTagNavigator::onLoop()
{
    ...
    try {
        ...
        double distance_remaining = nav2_util::geometry_utils::calculate_path_length(
            current_path, find_closest_pose_idx());

        // Default value for time remaining
        rclcpp::Duration estimated_time_remaining =
            rclcpp::Duration::from_seconds(0.0);

        // Get current speed
        geometry_msgs::msg::Twist current_odom = odom_smoother_->getTwist();
        double current_linear_speed =
            std::hypot(current_odom.linear.x, current_odom.linear.y);
        ...
        if ((std::abs(current_linear_speed) > 0.01) && (distance_remaining > 0.1)) {
            estimated_time_remaining = rclcpp::Duration::from_seconds(
                distance_remaining / std::abs(current_linear_speed));
        }
...
void ARTagNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
    ...
    if (goal->behavior_tree == bt_action_server_->getCurrentBTFilename() ||
        ...
        // pending goal if pending goal has an empty behavior_tree field, it requests
        // the default BT file accept the pending goal if the current goal is running
        // the default BT file
        ...
        RCLCPP_WARN(logger_,
                    ...
                    "as the one that the current goal is executing. Preemption with "
                    "a new BT is invalid "
                    "since it would require cancellation of the previous goal "
                    "instead of true preemption."
                    "\nCancel the current goal and send a new action request if you "
                    "want to use a "
...
void ARTagNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
    ...
    blackboard->set<geometry_msgs::msg::PoseStamped>(goal_blackboard_id_,
                                                     goal->pose);
}
...
PLUGINLIB_EXPORT_CLASS(ar_tag_navigate::ARTagNavigator, nav2_core::NavigatorBase)