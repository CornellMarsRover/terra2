...
class CmdVelPublisher(Node):
  ...
  def publish_msg(self):
      for port, sock in drive_sockets.items():
          ...
          msg = self.create_twist_stamped([lx, ly, rx, ry], frame_id=f"udp:{port}")
      ...
      if arm_packet is not None:
        ...
        if self.validate_drive_packet(arm_data, UDP_PORT_ARM, addr, kind="arm"):
          ...
          arm_msg = self.create_twist_stamped(
              [
                  lx,
                  ly,
                  arm_button_array[2],
      ...
      if mini_arm_packet is not None:
        ...
        if len(mini_arm_data_raw) != MINI_ARM_PACKET_LENGTH:
          self.logger.warn(
              f"Ignoring mini-arm packet from {addr[0]}:{addr[1]} on {UDP_PORT_MINI_ARM}: "
              f"expected {MINI_ARM_PACKET_LENGTH} bytes, got {len(mini_arm_data_raw)}",
  ...
  def validate_drive_packet(self, data, port, addr, kind="drive"):
    if len(data) < DRIVE_PACKET_LENGTH:
      self.logger.warn(
          ...
          f"expected at least {DRIVE_PACKET_LENGTH} bytes, got {len(data)}",
          throttle_duration_sec=2.0,
      )
      return False
    return True
