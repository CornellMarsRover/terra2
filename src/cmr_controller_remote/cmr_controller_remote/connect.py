...
class CmdVelPublisher(Node):
    ...
    def publish_msg(self):
        for port, sock in drive_sockets.items():
            ...
            self.publisher_.publish(
                self.create_twist_stamped(
                    [lx, ly, rx, ry],
                    frame_id=f"udp:{port}",
                )
            )
            self.button_publisher_.publish(
                self.create_button_message(button_array, dpad)
            )
        ...
        if mini_arm_packet is not None:
            ...
            self.mini_arm_publisher.publish(
                self.create_mini_arm_message(mini_arm_data)
            )
    ...
    def create_arm_joy_message(self, velocities, frame_id=""):
        ...
        joy_msg.buttons = [x, circle, triangle, square, l1, r1, 0, 0, 0, 0, 0, 0, 0]
        return joy_msg
    ...
    def decode_drive_buttons(self, button_array):
        ...
        for index, name in enumerate(names):
            if index < len(button_array):
                state[name] = int(button_array[index])
        return state
    ...
    def parse_controller_data(self, raw_data):
        lx = self.scale_value(raw_data[0], 0, 255, -2.5, 2.5)
        ly = self.scale_value(raw_data[1], 0, 255, -2.5, 2.5)
        rx = self.scale_value(raw_data[2], 0, 255, -2.5, 2.5)
        ry = self.scale_value(raw_data[3], 0, 255, -2.5, 2.5)
