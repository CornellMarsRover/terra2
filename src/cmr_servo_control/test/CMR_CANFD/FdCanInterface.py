...
class FdCanInterface:
    ...
    """
    def __init__(self, port: str = "COM16", baud: int = 115200, timeout: float = 1.0):
        self.port    = port
        self.baud    = baud
        self.timeout = timeout
        self.reader  = None  # asyncio.StreamReader
        self.writer  = None  # asyncio.StreamWriter
        self._seq    = 0     # 8-bit sequence counter

    async def open(self):
        """Open serial port & flush any boot messages."""
    ...
    async def _send_command(self, cmd: str) -> str:
        """Send a config or on/off command and wait for OK/ERR."""
    ...
    async def configure_bus(self):
        """Put the dongle into 1M/5M FD+BRS mode and turn CAN on."""
        for cmd in (
            "can off",
            "conf set can.bitrate 1000000",
            "conf set can.fdcan_frame on",
            "conf set can.fd_bitrate 5000000",
    ...
    async def write_frame(self, std_id: int, data_hex: str, flags="FB"):
        """
        Transmit one CAN-FD+BRS frame without waiting for an OK.
        - std_id:   0..0x7FF
        - data_hex: 16-char hex string (8 bytes)
        - flags:    e.g. "FB"
        """
    ...
    async def read_loop(self):
        """Continuously print any received CAN frames."""
        while True:
    ...
    async def request_response(self,
        ...
        """
        Send a 7-byte payload with an 8-bit seq# prefix, then wait for
        a reply whose first byte matches that seq#. Returns (resp_bytes, flags).
        ...
        # build 8-byte little-endian frame: seq + payload + padding
