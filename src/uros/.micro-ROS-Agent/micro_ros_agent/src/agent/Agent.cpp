...
Agent::Agent() : xrce_dds_agent_instance_(xrce_dds_agent_instance_.getInstance()) {}

bool Agent::create(int argc, char** argv)
{
    ...
    if (result) {
        ...
        on_create_participant(
            [&](const eprosima::fastdds::dds::DomainParticipant* participant)
                -> void {
                auto graph_manager_ =
                    find_or_create_graph_manager(participant->get_domain_id());
        ...
        on_create_datawriter(
            [&](const eprosima::fastdds::dds::DomainParticipant* participant,
                const eprosima::fastdds::dds::DataWriter* datawriter) -> void {
                auto graph_manager_ =
                    find_or_create_graph_manager(participant->get_domain_id());

                graph_manager_->add_datawriter(datawriter->guid(), participant,
                                               datawriter);
                graph_manager_->associate_entity(datawriter->guid(), participant,
                                                 dds::xrce::OBJK_DATAWRITER);
            });
        xrce_dds_agent_instance_.add_middleware_callback(
            eprosima::uxr::Middleware::Kind::FASTDDS,
        ...
        on_create_datareader(
            [&](const eprosima::fastdds::dds::DomainParticipant* participant,
                const eprosima::fastdds::dds::DataReader* datareader) -> void {
                auto graph_manager_ =
                    find_or_create_graph_manager(participant->get_domain_id());

                // TODO(jamoralp): Workaround for Fast-DDS bug #9977. Remove when
                // fixed
                ...
                graph_manager_->add_datareader(datareader_guid, participant,
                                               datareader);
                graph_manager_->associate_entity(datareader_guid, participant,
                                                 dds::xrce::OBJK_DATAREADER);
            });
        xrce_dds_agent_instance_.add_middleware_callback(
            eprosima::uxr::Middleware::Kind::FASTDDS,
        ...
        on_create_requester(
            [&](const eprosima::fastdds::dds::DomainParticipant* participant,
                ...
                // TODO(pablogs): Workaround for Fast-DDS bug #9977. Remove when
                // fixed
                const eprosima::fastrtps::rtps::InstanceHandle_t instance_handle =
                    datareader->get_instance_handle();
                const eprosima::fastrtps::rtps::GUID_t datareader_guid =
...
void Agent::run() { return xrce_dds_agent_instance_.run(); }

std::shared_ptr<graph_manager::GraphManager> Agent::find_or_create_graph_manager(
    eprosima::fastdds::dds::DomainId_t domain_id)
{
    ...
    if (it != graph_manager_map_.end()) {
        ...
        return graph_manager_map_
            .insert(std::make_pair(
                domain_id, std::make_shared<graph_manager::GraphManager>(domain_id)))
            .first->second;
    }
}
