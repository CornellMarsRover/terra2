...
class CmdVelSubscriber(Node):
    ...
    def gradually_increase_speed_angular(self, target_speed):
        ...
        if self.current_speed_angular < target_speed:
            self.current_speed_angular += dynamic_rate
            if self.current_speed_angular > target_speed:
                self.current_speed_angular = target_speed
    ...
    def listener_callback(self, msg):
        ...
        target_speed_angular = scale_value(msg.twist.angular.y, -2.5, 2.5, -53, 53)
        ...
        if msg.twist.angular.z != 0:
            back_right = byte_command_converter("drives", "back_right", None, -self.current_speed_angular, 1.5, None, 5.0, self.get_logger())
            front_right = byte_command_converter("drives", "front_right", None, -self.current_speed_angular, 1.5, None, 5.0, self.get_logger())
            front_left = byte_command_converter("drives", "front_left", None, self.current_speed_angular, 1.5, None, 5.0, self.get_logger())
            back_left = byte_command_converter("drives", "back_left", None, self.current_speed_angular, 1.5, None, 5.0, self.get_logger())
            send_number(self.serial_port, back_right)
            send_number(self.serial_port, front_right)
            send_number(self.serial_port, front_left)
            send_number(self.serial_port, back_left)
