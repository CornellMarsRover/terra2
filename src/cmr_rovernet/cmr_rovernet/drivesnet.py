...
class CmdVelSubscriber(Node):
    ...
    def __init__(self):
        ...
        # ---------------- MOTEUS: create transport + controllers ONCE ----------------
        ...
        # Use a single shared transport (matches your working script behavior)
        # Prefer /dev/serial/by-id/... later, but keep /dev/ttyACM0 for now.
...
# Drives (reuse controllers)
        send_moteus_command_sync(
            controller=self.ctrl_drive["FL"], motor=1,
            position=math.nan, drives_velocity=(-self.velocity * ws2),
            maximum_torque=self.MAX_TORQUE, velocity_limit=self.MOTOR_MAX_SPEED,
            accel_limit=self.MAX_ACCELERATION, ff_torque=0, logger=self.logger
        )
        send_moteus_command_sync(
            controller=self.ctrl_drive["BL"], motor=2,
            position=math.nan, drives_velocity=(-self.velocity * ws3),
            maximum_torque=self.MAX_TORQUE, velocity_limit=self.MOTOR_MAX_SPEED,
            accel_limit=self.MAX_ACCELERATION, ff_torque=0, logger=self.logger
        )
        send_moteus_command_sync(
            controller=self.ctrl_drive["FR"], motor=3,
            position=math.nan, drives_velocity=(self.velocity * ws4),
            maximum_torque=self.MAX_TORQUE, velocity_limit=self.MOTOR_MAX_SPEED,
            accel_limit=self.MAX_ACCELERATION, ff_torque=0, logger=self.logger
        )
        send_moteus_command_sync(
            controller=self.ctrl_drive["BR"], motor=4,
            position=math.nan, drives_velocity=(self.velocity * ws1),
            maximum_torque=self.MAX_TORQUE, velocity_limit=self.MOTOR_MAX_SPEED,
            accel_limit=self.MAX_ACCELERATION, ff_torque=0, logger=self.logger
        )
        ...
        # Steers (reuse controllers)
        send_moteus_command_sync(controller=self.ctrl_steer["FR"], motor=7, position=wa1,
                                 drives_velocity=None, maximum_torque=10,
                                 velocity_limit=60, accel_limit=40, ff_torque=None, logger=self.logger)
        send_moteus_command_sync(controller=self.ctrl_steer["BR"], motor=8, position=wa4,
                                 drives_velocity=None, maximum_torque=10,
                                 velocity_limit=60, accel_limit=40, ff_torque=None, logger=self.logger)
        send_moteus_command_sync(controller=self.ctrl_steer["BL"], motor=6, position=wa3,
                                 drives_velocity=None, maximum_torque=10,
                                 velocity_limit=60, accel_limit=40, ff_torque=None, logger=self.logger)
        send_moteus_command_sync(controller=self.ctrl_steer["FL"], motor=5, position=wa2,
                                 drives_velocity=None, maximum_torque=10,
                                 velocity_limit=60, accel_limit=40, ff_torque=None, logger=self.logger)
