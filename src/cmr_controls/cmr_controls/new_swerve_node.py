...
@dataclass
class LegacyAutonomyCommandState:
    mode: str = ""
    velocity: float = 0.0
    fl_angle_deg: float = 0.0
    fr_angle_deg: float = 0.0
    angular_z: float = 0.0
    updated_at: float = 0.0
...
def calculate_ackermann_topic_targets(
    velocity: float,
    fl_angle_deg: float,
    fr_angle_deg: float,
    drive_rps: float,
    ...
    speed_rps = max(-1.0, min(1.0, velocity)) * drive_rps
    fl_delta = fl_angle_deg * STEER_DEGREES_TO_POSITION
    fr_delta = fr_angle_deg * STEER_DEGREES_TO_POSITION

    return {
        "drive": {
            motor_id: FORWARD_SIGN[motor_id] * speed_rps
            for motor_id in DRIVE_IDS
        },
        "steer": {
            5: center_offsets[5] + STEER_SIGN[5] * fl_delta,
            7: center_offsets[7] + STEER_SIGN[7] * fr_delta,
            6: center_offsets[6],
            8: center_offsets[8],
        },
    }
...
def calculate_point_turn_topic_targets(
    angular_z: float,
    drive_rps: float,
    center_offsets: dict[int, float],
) -> dict[str, dict[int, float]]:
    omega = max(-1.0, min(1.0, angular_z))
    return calculate_swerve_targets(
        vx=0.0,
        vy=0.0,
        omega=omega,
        drive_rps=abs(omega) * drive_rps,
        center_offsets=center_offsets,
    )


async def command_target_motors_async(
    port_arg: str,
    timeout_s: float,
...
class UsamaControlRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._legacy_autonomy = LegacyAutonomyCommandState()
        ...
        self.create_subscription(
            ...
            self._autonomy_ackermann_cb,
            10,
        )
        self.create_subscription(
            Twist,
            "/autonomy/move/point_turn",
            self._autonomy_point_turn_cb,
            10,
        )
        ...
        self.get_logger().info(
            ...
            "/autonomy/move/ackerman, /autonomy/move/point_turn"
        )
        self.get_logger().info(
    ...
    def _autonomy_ackermann_cb(self, msg: AutonomyDrive) -> None:
        with self._lock:
            self._legacy_autonomy.mode = "ackerman"
            self._legacy_autonomy.velocity = float(msg.vel)
            self._legacy_autonomy.fl_angle_deg = float(msg.fl_angle)
            self._legacy_autonomy.fr_angle_deg = float(msg.fr_angle)
            self._legacy_autonomy.angular_z = 0.0
            self._legacy_autonomy.updated_at = time.time()

        self.get_logger().info(
            f"autonomy ackerman vel={msg.vel:+.3f} "
            f"fl={msg.fl_angle:+.2f} fr={msg.fr_angle:+.2f}",
            throttle_duration_sec=1.0,
        )

    def _autonomy_point_turn_cb(self, msg: Twist) -> None:
        with self._lock:
            self._legacy_autonomy.mode = "point_turn"
            self._legacy_autonomy.velocity = 0.0
            self._legacy_autonomy.fl_angle_deg = 0.0
            self._legacy_autonomy.fr_angle_deg = 0.0
            self._legacy_autonomy.angular_z = float(msg.angular.z)
            self._legacy_autonomy.updated_at = time.time()

        self.get_logger().info(
            f"autonomy point_turn angular_z={msg.angular.z:+.3f}",
            throttle_duration_sec=1.0,
        )
    ...
    def _worker_loop(self) -> None:
        ...
        while not self._shutdown.is_set():
            try:
                ...
                elif command["mode"] == "targets":
                    targets = command["targets"]
                    asyncio.run(
                        command_target_motors_async(
                            port_arg=self.port,
    ...
    def _select_command(self) -> dict[str, object]:
        ...
        with self._lock:
            ...
            legacy_autonomy = LegacyAutonomyCommandState(**self._legacy_autonomy.__dict__)
        ...
        legacy_autonomy_active = (
            bool(legacy_autonomy.mode)
            and (now - legacy_autonomy.updated_at) <= self.command_timeout_s
        )
        ...
        if legacy_autonomy_active and self.autonomy_priority:
            return self._legacy_autonomy_command(legacy_autonomy)
        if autonomy_active and self.autonomy_priority:
            return self._autonomy_command(autonomy)
        if manual_active:
            return self._manual_command(manual)
        if legacy_autonomy_active:
            return self._legacy_autonomy_command(legacy_autonomy)
        if autonomy_active:
            return self._autonomy_command(autonomy)
    ...
    def _legacy_autonomy_command(self, command: LegacyAutonomyCommandState) -> dict[str, object]:
        if command.mode == "ackerman":
            targets = calculate_ackermann_topic_targets(
                velocity=command.velocity,
                fl_angle_deg=command.fl_angle_deg,
                fr_angle_deg=command.fr_angle_deg,
                drive_rps=self.drive_rps,
                center_offsets=self._steer_center_offsets,
            )
            return {
                "mode": "targets",
                "source": "autonomy_ackerman",
                "targets": targets,
            }

        targets = calculate_point_turn_topic_targets(
