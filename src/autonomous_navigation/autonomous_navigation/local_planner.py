...
class LocalPlannerNode(Node):
    def __init__(self):
        ...
        if self.visualize:
            ...
            rr.connect_tcp("192.168.1.101:9876")
    ...
    def plot_grid_rerun(self):
        ...
        if self.next_target is not None:
            points.append([self.next_target[0] - self.robot_position[0], self.next_target[1] - self.robot_position[1], 0])
            colors.append([0, 255, 0])
            radii.append(0.225)
    ...
    def compute_path(self, gap=3):
        ...
        min_y = start[1] - half_width - 1.0
        max_y = start[1] + half_width + 1.0
