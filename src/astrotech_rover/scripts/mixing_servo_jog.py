...
the mixing-chamber preset calibration (open question 6 in
``src/astrotech_rover/third_party/astrotech_canfd/API_NOTES.md``):
...
``src/astrotech_rover/third_party/astrotech_canfd/servo_bdc_control_ms/test/CMR_CANFD/``.
We duplicate the 4 populated bytes here rather than importing the
vendored library, per ``src/astrotech_rover/third_party/astrotech_canfd/NOTICE.md``
...
(the auger Foxglove panel, ``src/astrotech_rover/scripts/auger_slight_move.py``, etc.) at
...
# (``src/astrotech_rover/third_party/astrotech_canfd/API_NOTES.md``) both say 500 kbit/s, but the
...
def _do_probe(cli: FdCanCli, args: argparse.Namespace) -> int:
    ...
    print(
        "(reminder: per src/astrotech_rover/third_party/astrotech_canfd/API_NOTES.md the boards "
