...
def main() -> int:
    ...
    try:
        while True:
            ...
            # Match cmr_msgs/ControllerReading button_array indices:
            # L1, R1, L2, R2, SQUARE, X/CROSS, CIRCLE, TRIANGLE.
            packet = bytes(
                [
            ...
            sys.stdout.write(
                ...
                f"l1={int(bool(slow_button))} r1={int(bool(boost_button))} "
                f"l2={int(bool(l2_button))} r2={int(bool(r2_button))}"
                f"{raw_axis_suffix}   "
            )
            sys.stdout.flush()
            time.sleep(period)
