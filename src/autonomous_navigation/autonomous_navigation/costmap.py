...
class CostmapNode(Node):
    def __init__(self):
        ...
        # Autonomy game object position (store to avoid marking game object as obstacle)
    ...
    def object_callback(self, msg):
        """
        Position of autonomy game object (to avoid marking as obstacle)
        """
        N = 1
        x, y = msg.linear.x, msg.linear.y
        ...
        for dx in range(-1*N, N, 1):
            for dy in range(-1*N, N, 1):
                ...
                self.grid_dict[(safe_x, safe_y)] = 0
