...
async def query_timeout(ctrl: moteus.Controller, timeout_s: float = QUERY_TIMEOUT_S):
    try:
        return await asyncio.wait_for(ctrl.query(), timeout=timeout_s)
    except Exception:
        return None
...
async def rezero_and_sync_steers(ctrls, steer_angle_est):
    ...
    states = await asyncio.gather(
        *[query_timeout(ctrls[f"{s}_steer"], timeout_s=0.2) for s in SWERVE_IDS],
        return_exceptions=False
    )

    for side, st in zip(SWERVE_IDS.keys(), states):
        if st is None:
            print(f"[WARN] steer query failed for {side}")
            continue
    ...
    try:
        while True:
            ...
            for side in SWERVE_IDS:
                ...
                # If wheel speed is tiny, don't hunt steering
                if spd < WHEEL_SPEED_EPS:
                    new_angle = steer_angle_est[side]
                    opt_spd = 0.0
                else:
                ...
                # ---- DRIVE SPEED SCALE (NEW) ----
                cmd_drive_vel = DRIVE_SIGN[side] * opt_spd * DRIVE_MAX_RPS
                ...
                if drv_key in faulted:
                    ...
                    tasks.append(send_position_noreply(
                        ...
                        accel_limit=DRIVE_ACCEL_LIMIT,  # <-- not ACCEL_RATE
                        watchdog_timeout=0.2,
                    ))
            ...
            if tick % TELEMETRY_EVERY_N_TICKS == 0:
                ...
                states = await asyncio.gather(
                    *[query_timeout(ctrls[n], timeout_s=QUERY_TIMEOUT_S) for n in names],
                    return_exceptions=False
                )
