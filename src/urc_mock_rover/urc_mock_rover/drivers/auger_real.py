...
Drop-in replacement for ``MockAugerDriver`` (same ``__init__(node, cfg)``
signature, same subscribe/publish topics). Selected via the
``URC_AUGER_REAL`` env var in :mod:`urc_mock_rover.mock_rover_node`; mock
remains the default so devs without hardware can still ``ros2 launch``.
...
``scripts/auger_slight_move.py`` was validated against during the auger
...
drum. The bring-up script ``scripts/auger_slight_move.py`` intentionally
